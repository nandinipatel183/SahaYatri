  ...
  return (
    <>
      <Routes>
        ...
        <Route 
          path="/map" 
          element={user ? <MapNavigation /> : <Navigate to="/login" />} 
        />

        <Route path="/match" element={<MatchFaces />} />

        {/* CORRECT DASHBOARD ROUTES */}
        <Route path="/dashboard-user" element={user?.role === "USER" ? <UserDashboard /> : <Navigate to="/login" />} />
<Route path="/volunteer-dashboard" element={user?.role === "VOLUNTEER" ? <VolunteerDashboard /> : <Navigate to="/login" />} />
