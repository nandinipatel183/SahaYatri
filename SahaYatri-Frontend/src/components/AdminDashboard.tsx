...
type RoleString = "ADMIN" | "VOLUNTEER" | "USER" | "PUBLIC";
...
const AdminDashboard: React.FC = () => {
  ...
  const [tab, setTab] = useState<"overview" | "users" | "reports" | "analytics" | "matches">("overview");
  ...
  const [trendData, setTrendData] = useState<{ date: string; reports: number }[]>([]);
  ...
  async function fetchUsers() {
    ...
    try {
      ...
      // metrics update
      const volunteers = normalized.filter((x) => x.role === "VOLUNTEER" && x.approved).length;
      const pending = normalized.filter((x) => !x.approved).length;
      setMetrics((m) => ({ ...m, totalUsers: normalized.length, volunteers, pendingApprovals: pending }));
    } catch (err: any) {
  ...
  async function changeRole(userId: number, role: RoleString) {
    ...
    try {
      await apiFetch(`/api/admin/role/${userId}?role=${role}`, { method: "PATCH" });
  ...
  async function fetchReportsAndItems() {
    try {
      ...
      const lp = lpR.status === "fulfilled" && Array.isArray(lpR.value) ? lpR.value : [];
      const fp = fpR.status === "fulfilled" && Array.isArray(fpR.value) ? fpR.value : [];
      const li = liR.status === "fulfilled" && Array.isArray(liR.value) ? liR.value : [];
      const fi = fiR.status === "fulfilled" && Array.isArray(fiR.value) ? fiR.value : [];
      ...
      const normalize = (arr: any[], type: string) =>
        (Array.isArray(arr) ? arr : []).map((r) => ({
          ...
          location: r.lastSeenLocation || r.foundLocation || r.location || "Unknown",
      ...
      if (mtR.status === "fulfilled") {
        ...
        if (Array.isArray(mtVal)) {
          ...
          const ppl = Array.isArray(mtVal.peopleMatches) ? mtVal.peopleMatches : [];
          const itms = Array.isArray(mtVal.itemMatches) ? mtVal.itemMatches : [];
  ...
  function pushNotification(message: string) {
    setNotifications((s) => [{ id: Date.now().toString(), message, time: new Date().toLocaleTimeString() }, ...s].slice(0, 6));
  }
  ...
  function exportUsersCsv() {
    ...
    const header = ["id", "name", "email", "phone", "role", "approved", "createdAt"];
    const rows = users.map((u) => [u.id, u.name ?? "", u.email, u.phone ?? "", u.role, u.approved ? "true" : "false", u.createdAt ?? ""]);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((u) => (u.name || "").toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || String(u.id) === q);
  }, [users, query]);
      ...
      <div className="max-w-7xl mx-auto px-4 pt-28 pb-12">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">Manage users, reports, AI matches & system data</p>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => exportUsersCsv()} className="px-3 py-2 bg-white rounded shadow hover:shadow-md inline-flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {notifications.length > 0 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full px-1.5 py-0.5">{notifications.length}</span>}
            </div>
          </div>
        </div>
                ...
                <div style={{ height: 260 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={trendData}>
                      ...
                      <YAxis />
                      <Tooltip />
                      <Line type="monotone" dataKey="reports" stroke="#7C3AED" strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
                ...
                <div style={{ height: 260 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={[
                          { name: "Lost Persons", value: metrics.lostPersonCount },
                          { name: "Found Persons", value: metrics.foundPersonCount },
        ...
        {tab === "users" && (
          <div className="bg-white p-6 rounded-2xl shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <input placeholder="Search name, email..." value={query} onChange={(e) => setQuery(e.target.value)} className="pl-10 pr-3 py-2 border rounded-lg w-72" />
                  <Search className="absolute left-3 top-2.5 text-gray-400 w-4 h-4" />
                </div>
                <button onClick={() => fetchUsers()} className="px-3 py-2 bg-indigo-600 text-white rounded-lg">Refresh</button>
              </div>
            </div>

            {usersLoading ? <p>Loading users...</p> : usersError ? <p className="text-red-600">{usersError}</p> : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-sm text-gray-500">Approved</th>
                  ...
                  <tbody>
                    {filteredUsers.map((u) => (
                      <tr key={u.id} className="border-b">
                        ...
                        <td className="px-4 py-3">{u.approved ? <span className="text-green-600">Approved</span> : <span className="text-yellow-600">Pending</span>}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {!u.approved && <button onClick={() => approveUser(u.id)} className="px-3 py-1 bg-green-600 text-white rounded"><UserCheck className="inline w-4 h-4 mr-1" />Approve</button>}
                            <select value={u.role} onChange={(e) => changeRole(u.id, e.target.value as RoleString)} className="px-3 py-1 border rounded">
                              <option value="USER">Public</option>
                              <option value="VOLUNTEER">Volunteer</option>
                              <option value="ADMIN">Admin</option>
                            </select>
                            <button onClick={() => deleteUser(u.id)} className="px-3 py-1 bg-red-100 text-red-700 rounded"><Trash2 className="inline w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
                ...
                {filteredUsers.length === 0 && <p className="text-gray-500 mt-4">No users found.</p>}
              </div>
            )}
          </div>
        )}
        ...
        {tab === "reports" && (
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="bg-white p-6 rounded-2xl shadow">
              <h3 className="text-lg font-semibold mb-4">Latest Person Reports</h3>
              <div className="space-y-3">
                {[...foundPeople.slice(0, 6), ...lostPeople.slice(0, 6)].map((r) => (
                  <div key={`${r.type}-${r.id}`} className="p-3 border rounded-lg flex justify-between items-center">
                    <div>
                      <div className="font-semibold">{r.title}</div>
                      <div className="text-sm text-gray-500">{r.location}</div>
                    </div>
                    <div className="text-sm text-gray-400">{r.createdAt ? String(r.createdAt).slice(0, 16) : ""}</div>
                  </div>
                ))}
                {foundPeople.length + lostPeople.length === 0 && <p className="text-gray-500">No person reports yet.</p>}
              </div>
            </div>

            <div className="bg-white p-6 rounded-2xl shadow">
              <h3 className="text-lg font-semibold mb-4">Latest Item Reports</h3>
              <div className="space-y-3">
                {[...foundItems.slice(0, 6), ...lostItems.slice(0, 6)].map((r) => (
                  <div key={`${r.type}-${r.id}`} className="p-3 border rounded-lg flex justify-between items-center">
                    <div>
                      <div className="font-semibold">{r.title}</div>
                      <div className="text-sm text-gray-500">{r.location}</div>
                ...
                {foundItems.length + lostItems.length === 0 && <p className="text-gray-500">No item reports yet.</p>}
              </div>
            </div>
          </div>
        )}
        ...
        {tab === "analytics" && (
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="bg-white p-6 rounded-2xl shadow">
              <h3 className="text-lg font-semibold mb-4">Reports Trend (last 7)</h3>
            ...
            <div className="bg-white p-6 rounded-2xl shadow">
              <h3 className="text-lg font-semibold mb-4">Case Type Distribution</h3>
              <div style={{ height: 300 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
          ...
          <div className="bg-white p-6 rounded-2xl shadow">
            ...
            {safeMatches.length === 0 ? (
              ...
              safeMatches.map((m: any, i: number) => (
                <div key={m.id ?? i} className="border rounded-lg p-4 mb-3 grid grid-cols-1 md:grid-cols-5 gap-3 items-center">
                  <div className="col-span-1 md:col-span-3">
                    <div className="font-semibold">Match #{m.id ?? i}</div>
                    <div className="text-sm text-gray-500">Type: {m.type || m.matchType || (m.lostId && m.foundId ? "person/item" : "unknown")}</div>
                    <div className="text-sm text-gray-500">Time: {m.matchedAt || m.time || m.createdAt || "N/A"}</div>
                  </div>

                  <div className="col-span-1 flex gap-2 items-center">
                    <div className="text-sm"><strong>Confidence</strong><div className="text-2xl font-bold">{((m.confidence ?? m.similarity ?? m.score) || 0).toFixed(2)}%</div></div>
                  </div>

                  <div className="col-span-1 md:col-span-1 flex gap-2 justify-end">
                    {m.lostImage && <img alt="lost" src={m.lostImage} className="w-20 h-20 object-cover rounded" />}
                    {m.foundImage && <img alt="found" src={m.foundImage} className="w-20 h-20 object-cover rounded" />}
                    {!m.lostImage && m.leftImageUrl && <img alt="left" src={m.leftImageUrl} className="w-20 h-20 object-cover rounded" />}
                    {!m.foundImage && m.rightImageUrl && <img alt="right" src={m.rightImageUrl} className="w-20 h-20 object-cover rounded" />}
                  </div>
